...
} from 'firebase/auth'; // 補齊登出和 Google 登入功能
import { 
    getFirestore, doc, collection, onSnapshot, addDoc, updateDoc, deleteDoc, 
    query, orderBy, serverTimestamp, where, getDocs, runTransaction, arrayUnion, arrayRemove // 補齊 arrayUnion/arrayRemove
...
import { 
    ...
    User, Settings, ClipboardList, GripVertical, AlertTriangle, ChevronDown, ChevronUp, Send, UserPlus
} from 'lucide-react';
...
if (typeof __firebase_config !== 'undefined') {
    ...
    // *** 修正後的環境變數讀取方式：統一使用 process.env ***
    // 雖然您的 .env 檔案可能使用 VITE_ 前綴，但在許多建構環境中，它們最終會被轉換為 process.env
    
    // 注意：這裡使用 process.env 來確保與舊版目標環境相容性
    firebaseConfig = {
    ...
    if (!firebaseConfig.apiKey || !firebaseConfig.projectId) {
        // 如果配置仍然遺失，發出警告
        console.warn("Firebase Configuration Warning: Environment variables (process.env.VITE_FIREBASE_API_KEY, etc.) are missing. Using minimal config or default values.");
    }
}
...
// --- 這裡開始是應用程式的組件和邏輯 (與先前版本相同，但已新增到這個文件中) ---
                    ...
                    {[...Array(days)].map((_, index) => (
                        <div key={index} className="p-3 bg-gray-50 dark:bg-gray-700 rounded-lg flex items-center justify-between">
                            ...
                            <span className="text-sm text-indigo-500 cursor-pointer hover:text-indigo-400 flex items-center">
                                <MapPin className="w-4 h-4 mr-1" /> 查看/編輯
                            </span>
                        </div>
                    ))}
                </div>
...
const TripDetail = ({ tripId, onBack, userId, authReady }) => {
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(tripRef, (docSnap) => {
            if (docSnap.exists()) {
                ...
                // onBack(); // 避免在快照失敗時無限循環，但應在真實環境中回退
            }
            setIsLoading(false);
        }, (err) => {
    ...
    const handleSaveTrip = async () => {
        if (!trip.name || !trip.startDate || !trip.endDate) {
            // 使用自訂模態框取代 alert
            // 在這裡我們使用一個簡單的訊息，因為我們沒有實現完整的 Modal
            alert("請填寫所有必要的行程資訊。");
            return;
        }
        try {
