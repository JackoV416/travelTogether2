...
const VERSION_HISTORY = [
    { ver: "V0.6.1", date: "10/12/2025", desc: "旅遊資訊中心修正與新功能加入", details: "1. 修正旅遊資訊中心的顯示BUG。\n2. 新增建立行程的顯示BUG修正。" }, 
    { ver: "V0.6.0", date: "01/12/2025", desc: "Beta 版：首頁、AI、簽證、保險、地圖等全方位升級", details: "1. 所有日期以 DD/MM/YYYY 顯示，建立行程支援多國多城多選。\n2. 首頁卡片新增即時天氣與衣著提示、旅遊資訊中心含更多連結。\n3. 簽證區分公開狀態與個人詳情，教學資料補齊 5 天內容。\n4. AI 領隊提供真實建議，可加入行程與交通推薦。\n5. 機票、交通卡片顯示航空公司 / 交通 Logo，保險提供 AI 建議與 Visit Japan Web 指引。\n6. 每日行程摘要、交通建議、地圖視圖集合全部行程地點。" },
    { ver: "V0.5.0", date: "26/11/2025", desc: "版本視窗、簽證資訊與彈窗體驗更新", details: "1. 新增版本按鈕與詳細紀錄視窗。\n2. 補齊簽證分頁內容與權限顯示。\n3. 優化加入行程等彈窗文案。" },
    { ver: "V0.4.0", date: "26/11/2025", desc: "真實東京行程教學、新增項目支援稅務/轉機", details: "1. 教學模式資料替換為真實東京5日遊。\n2. 新增項目可填寫稅金、退稅、航班轉機。\n3. 緊急資訊根據用戶所在地顯示正確辦事處。" },
];
    ...
    return (
        <div className="fixed inset-0 bg-black/60 z-[80] flex items-center justify-center p-4 backdrop-blur-sm animate-fade-in">
            <div className={`w-full max-w-md rounded-2xl p-6 ${isDarkMode ? 'bg-gray-800 text-white' : 'bg-white text-gray-900'} shadow-2xl`}>
                <h3 className="text-xl font-bold mb-4">版本紀錄（Beta 測試中）</h3>
                <div className="space-y-4 max-h-[60vh] overflow-y-auto custom-scrollbar">
                    {VERSION_HISTORY.map((v,i)=>(<div key={i} className="border-l-2 border-indigo-500 pl-4 pb-2">
                        <div className="font-bold">
                            {v.ver} <span className="text-xs opacity-50">{v.date}</span>
                        </div>
                        <p className="text-sm opacity-80 whitespace-pre-wrap">{v.desc}</p>
                        {v.details && (
...
const App = () => {
    ...
    if (!user) return <LandingPage onLogin={()=>signInWithPopup(auth, googleProvider)}/>;
     ...
     // 添加以下代碼來顯示 APP_VERSION
     const APP_VERSION = VERSION_HISTORY[0].ver;
